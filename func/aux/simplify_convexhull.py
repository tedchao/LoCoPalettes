...
def remove_point_by_finding_smallest_adding_area( hull, pts ):
    areas = []
    for i in range( pts.shape[0] ):
        mask = [ True ] * pts.shape[0]
        mask[i] = False
        points_stay = pts[ mask ]
        new_hull = ConvexHull( points_stay )
        areas.append( new_hull.area )
    smallest_adding_pt_indx = np.argmax( np.array( areas ) )
    
    mask = [ True ] * pts.shape[0]
    mask[ smallest_adding_pt_indx ] = False
    return pts[ mask ]

def simplified_2d_convex_hull( num_colors, hull, pts ):
    ###
    ### Given: palette size, convexhull for original data
    ###         and convexhull points for original data
    ### Return: palette
    print( 'original vertices number:', pts.shape[0] )
    if num_colors < pts.shape[0]:
        N = pts.shape[0] - num_colors
        for i in range( N ):
            pts = remove_point_by_finding_smallest_adding_area( hull, pts )
            hull = ConvexHull( pts )
    return pts
            
### ideally: def simplified_convex_hull(num_colors, hvertices, hfaces)
def simplified_convex_hull(num_colors, vertices, faces, objfile, verbose=True, store=True):
    ...
    if num_colors < len( mesh.vs ):
        ...
        newhull = ConvexHull(mesh.vs)   # new convex hull (to debug cuz the below `newhull` has reference before assignment )
    
    if store:
        hvertices=newhull.points[newhull.vertices]
        points_index=-1*np.ones(newhull.points.shape[0],dtype=np.int32)
        points_index[newhull.vertices]=np.arange(len(newhull.vertices))
        hfaces=np.array([points_index[hface] for hface in newhull.simplices])+1
        
        myfile=open(objfile + '-rawconvexhull.obj','w')
        for index in range(hvertices.shape[0]):
            myfile.write('v '+str(hvertices[index][0])+' '+str(hvertices[index][1])+' '+str(hvertices[index][2])+'\n')
        for index in range(hfaces.shape[0]):
            myfile.write('f '+str(hfaces[index][0])+' '+str(hfaces[index][1])+' '+str(hfaces[index][2])+'\n')
        ...
        with open( objfile + '-rawconvexhull.json', 'w' ) as myfile_origin:
            json.dump({'vs': (255*hvertices).tolist(),'faces': (255*newhull.points[ newhull.simplices ]).tolist()}, myfile_origin, indent = 4 )
